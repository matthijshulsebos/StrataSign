...
for (dataset_name in dataset_names_to_process) {
  for (version in versions_to_process) {
    ...
    cat(paste("Processing SVM RBF for dataset:", dataset_name, "| version:", version, "| type:", dataset_type, "\n"))
    ...
    # Train SVM with RBF kernel (the only substantial change from linear version)
    svm_model_rbf <- svm(X_train_selected_df, y_train, kernel = "radial", probability = TRUE, scale = FALSE)
    ...
    y_pred_svm_prob <- predict(svm_model_rbf, X_test_selected_df, probability = TRUE)

    # Probabilities for sample to be the positive class which is tumour
    y_prob_values <- attr(y_pred_svm_prob, "probabilities")[, "1"]

    # Create factors out of predictions and test labels
    ...
    # For feature importance with RBF kernel, we can't use coefficients like in linear SVM
    # Instead, we'll use the ANOVA F-values as a proxy for feature importance
    cat("Creating feature importance for RBF model based on ANOVA F-values...\n")
    
    # Store feature importance in a data frame using F-values for the selected features
    feature_importance <- data.frame(
      Feature = selected_feature_names,
      Value = f_values[ordered_indices[1:num_to_actually_select]],
      stringsAsFactors = FALSE
    )
    
    # Sort by importance
    feature_importance$Abs_Value <- abs(feature_importance$Value)
    feature_importance <- feature_importance[order(feature_importance$Abs_Value, decreasing = TRUE), ]
    ...
    write_csv(feature_importance[, c("Feature", "Value")], 
...
cat(paste("SVM RBF analysis completed successfully for dataset type:", dataset_type, "\n"))
