...
output_base_dir <- file.path("output", "2. models", dataset_type, "randomforest") 
...
for (dataset_name in dataset_names_to_process) {
  for (version in versions_to_process) {
    ...
    # Save model
    saveRDS(rf_fit, file.path(current_output_dir, 
            paste0("randomforest_model_", dataset_name, "_", version, ".rds")))
  }
}

# Save performance summary
summary_filename <- file.path(summary_dir, "randomforest_performance_summary.csv")
write_csv(performance_summary, summary_filename)
cat("Saved performance summary:", summary_filename, "\n")

print(paste("Random Forest analysis completed successfully for dataset:", dataset_type))
