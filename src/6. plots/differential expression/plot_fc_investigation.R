...
# Creates a boxplot to compare expression between tissue types
create_boxplot <- function(data, x, y, fill, title, subtitle, ylabel) {
  p <- ggplot(data, aes(x = {{x}}, y = {{y}}, fill = {{fill}})) +
...
main <- function() {
  ...
  for (i in 1:nrow(feature_info)) {
    ...
    save_plot(raw_boxplot, paste0(feature_clean, "_raw_expression_boxplot.png"))
    ...
    if (!is.null(metabolic_data) && feature %in% colnames(metabolic_data)) {
      ...
      save_plot(meta_boxplot, paste0("metabolic_", feature_clean, "_boxplot.png"))
